...
for subset1 in itertools.combinations(node, combinationNumber):
    for k in range(0, combinationNumber + 1):
        for subset2 in itertools.combinations(subset1, k):
            ...
            res = attr_cy.run(samples=100000, steps=100, debug=False, progress=True, on_states=nodeOnList, off_states=nodeOffList)
            combination["On" + str(nodeOnList) + "Off" + str(nodeOffList)] = res

json.dump(combination, open('CK_application_to_r10.json', 'w'), indent=4)